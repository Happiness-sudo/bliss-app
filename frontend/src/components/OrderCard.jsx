...
const statusColor = {
  ...
  sent_to_client: "text-ink dark:text-[#e9e4d8]",
  paid: "text-sage",
};

export default function OrderCard({ order, action }) {
  return (
    <div className="flex items-start gap-4 rounded border border-line dark:border-[#333b47] bg-white/60 dark:bg-[#1e242e]/60 p-5">
      <div className="mt-1 h-full w-1 self-stretch rounded-full bg-amber/70" />
      <div className="flex-1">
        <div className="flex items-start justify-between gap-3">
          <Link to={`/orders/${order.id}`} className="font-display text-lg text-ink dark:text-[#e9e4d8] hover:text-amber">
            #{order.order_number}
          </Link>
          <span className={`whitespace-nowrap text-sm ${statusColor[order.status] || "text-charcoal/60 dark:text-[#c9c2b0]/60"}`}>
        ...
        <p className="mt-3 text-sm leading-relaxed text-charcoal/85 dark:text-[#c9c2b0]/85">{order.instructions}</p>
        <div className="mt-4 flex items-center gap-3">
          <Link to={`/orders/${order.id}`} className="text-xs text-charcoal/50 dark:text-[#c9c2b0]/50 hover:text-amber">
            View details &rarr;
          </Link>
          {action}
        </div>
